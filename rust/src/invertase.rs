...
pub fn get_biomass(alpha: f64) -> f64 {
    let (d,     supply, omega, gamma, epsilon) = (
        0.5f64, 1f64,   0.1f64, 1f64,  2f64);
    let x1 = alpha - 1.0;
    let x2 = x1 * gamma * omega - alpha * epsilon;
    let x3 = x1 * supply * x2 * gamma;

    let sol: f64 = (
        -2.0 * x3 - (
            (2.0 * x3 + d.powi(2) * (-alpha * epsilon + x1 * gamma * (omega - 1.0))).powi(2)
            - 8.0 * d.powi(2) * supply * x1.powi(2) * x2 * gamma.powi(2) * (omega - 1.0)
        ).sqrt()
        + d.powi(2) * (alpha * epsilon - x1 * gamma * (1.0 + omega))
    ) /
    (-2.0 * d * alpha * epsilon + 2.0 * d * x1 * gamma * omega);
