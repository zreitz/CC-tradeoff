---
title: "Sweep of mutation size sigma"
output: 
  html_document
---
R code used to generate Figures 5E, 5F, S6A
...
```{r setup}
config = "rust_config.json" 
vars <- c(1e-3, 1e-2, 1e-1, 1)
results <- lapply(vars, function(var) {
  desc <- sprintf("vary_stdev_%s__mort_1.5", var)
  result <- paste0("results\\stdev_sweep\\", desc, ".json")
...
plts <- lapply(results, function(x) {
  ...
  comparePlot(points, lims = c(50000, 60000)) 
})

plts <- lapply(seq_along(vars), function(i) {
  plt <- plts[[i]] + labs(subtitle = paste0("σ = ", vars[i], " (υ = 0)"))
  plt
}) 
...
results <- lapply(vars, function(var) {
  ...
  result <- paste0("results\\stdev_sweep_long\\", desc, ".json")
  if (!file.exists(result)) {
    writeConfig(
    config,
    desc,
