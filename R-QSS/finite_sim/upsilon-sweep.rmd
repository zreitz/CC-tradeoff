---
title: "Sweep of immigration rate upsilon"
output: 
  html_document
---
R code used to generate Figures 5G, S6B
...
```{r setup}
config = "rust_config.json" 
vars <- c(1e-4, 1e-3, 1e-2, 1e-1)
results <- lapply(vars, function(var) {
  desc <- sprintf("vary_unif_%s__mort_1.5_stdev_1e-3", var)
  result <- paste0("results\\uniform_sweep\\", desc, ".json")
...
plts <- lapply(results, function(x) {
  ...
  comparePlot(points, lims = c(90000, 100000))  
})

plts <- lapply(seq_along(vars), function(i) {
  plt <- plts[[i]] + labs(subtitle = paste0("υ = ", vars[i], " (σ = 0.001)"))
  plt
}) 
...
results <- lapply(vars, function(var) {
  desc <- sprintf("vary_unif_%s__mort_1.5_stdev_1e-3", var)
  result <- paste0("results\\uniform_sweep_long\\", desc, ".json")
  if (!file.exists(result)) {
    writeConfig(
    config,
    desc,
